...
public class ImportService {
    ...
    @Async("asyncExecutor")
    public CompletableFuture<Boolean> initImportingPatients() throws InterruptedException {
            ...
            for (int i = startCounting; i < (customerSheet.getLastRowNum() +1) ; i++) {
                ...
                if(row != null) {
                    ...
                    try {
                        ...
                        feedback.alertInformation("No ha podido guardar el paciente");
                        Logger.getLogger(ImportService.class.getName()).log(Level.SEVERE, null, ex);
                    }

                }else{
                    feedback.alertInformation("Error con la hoja de excel");
                    break;
                }
            }
            result = true;
